...
// 1. can get coordinates but not pass to destination 
// 2. the calcuation function seems like borken if we use this one....
...
function renderMap(filter_condition) {
  ...
  navigator.geolocation.getCurrentPosition(function (lation) {
    ...
    // copy from here  -- get location from the map via click the map
...
function getRoute(end) {
  ...
  var start = [39.7452, -104.9922];
  var url = 'https://api.mapbox.com/directions/v5/mapbox/driving/' + current_lat + ',' + current_lng + ';' + latlng.lat + ',' + latlng.lng + '?steps=true&geometries=geojson&access_token=' + mapboxgl.accessToken;
